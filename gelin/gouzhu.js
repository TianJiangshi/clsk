export function initGouzhu(lib, game, ui, get, ai, _status, datasrc) {
	lib.init.css(lib.assetURL + 'extension/' + datasrc + '/gelin', 'gouzhu');
	//触发卡牌系统
	lib.skill.gl_triggerCard = {
		...
		onChooseToUse(event) {
			...
			if (evt.name == 'gl_triggerCard') event.set('gl_trigger', evt._trigger);
		},
		filter: function(event, player, name) {
			var cards = lib.skill.gl_triggerCard.cards;
			if (!cards[name]) return false;
			_status.event.gl_trigger = event;
			for (var i of cards[name]) {
				...
				if (!lib.card[i].notarget && !game.hasPlayer(function(current) {
	...
	if (!lib.gl_gouzhuList) lib.gl_gouzhuList = [];
	game.gl_triggerUp = function(pack) {
		...
		for (var name of lib.cardPack[pack]) {
			card = lib.card[name];
			if (card.gl_material) lib.gl_gouzhuList.add(name);
	...
	lib.element.card.inits.push(function(card) {
		...
		if (Object.keys(types).length == 1 && types.gl_magic) {
			str += '★' + types.gl_magic;
			if (types.gl_magic == Infinity) str = '★?';
		} else {
	...
	lib.element.player.gl_update = function() {
		game.broadcast(function(player, gl_mp, gl_maxMp, type, color, color2) {
			...
			player.$gl_update();
		}, this, this.gl_mp, this.gl_maxMp, this.node.gl_mp.type, this.node.gl_mp.color, this.node.gl_mp.color2);
		this.$gl_update();
	}
	lib.element.player.$gl_update = function() {
		var num;
	...
	lib.element.content.gl_useGouzhu = function() {
		...
		"step 2"
		var directresult = [],
			cards = get.gl_gouzhuCards(player);

		var cost = {};
		for (var i of event.cards) {
			cost[i[2]] = get.gl_material(i[2], player);
		}
		var directresult = [],
			cards = get.gl_gouzhuCards(player);
		//是个说法，不过为了适配新代码得改一下
		for (var name in cost) {
			for (var i of cost[name]) {
				if (get.ly_selectMaterial(player, name, i)[0] > 0) {
					directresult = null;
					break;
				}
				directresult.addArray(cards.filter(card => get.ly_gouzhuEnabled(player, card, name, i)));
			}
		}
	...
	lib.element.content.gl_gouzhu = function() {
		...
		if (lose_list.length || discard.length) {
			game.loseAsync({
				lose_list: lose_list,
				discard: discard,
				cards: cards,
				...
				//这个部分如果之后需要复用可以单独封装出去，这里就先偷个懒吧
				//各自的持有者失去各自的牌
				for (var i = 0; i < event.lose_list.length; i++) {
					var next = event.lose_list[i][0].lose(event.lose_list[i][1], ui.ordering).set("type", "ly_gouzhu").set("forceDie", true);
					next.relatedEvent = event.getParent();
					next.getlx = false;
				}
				//找不到持有者的统一直接处理
				if (event.discard.length) game.cardsGotoOrdering(event.discard);
		...
		player.gain(event.resultCards).gaintag = event.gaintag;
		event.num = 0;
		'step 2'
	...
	//获取素材限制
	get.gl_material = function(card, player) {
		...
		var material;
		if (lib.card[card.name].gl_material) material = lib.card[card.name].gl_material.slice();
		if (!Array.isArray(material)) material = [];
		if (owner) {
			game.checkMod(owner, material, card.name, 'gl_filterGouzhu', owner);
		}
		return material;
	}
	//获取单张卡牌是否满足构筑需求
	get.gl_gouzhuEnabled = function(player, card, name, cost, cards) {
		const mod = game.checkMod(player, card, name, cost, cards, 'unchanged', 'gl_gouzhuEnabled', player);
		if (typeof mod == 'boolean') return mod;
		return cost.filter(card, player, cards, name);
	}
	//获取素材限制的需求数量
	get.gl_selectMaterial = function(player, name, cost) {
		var select = get.select(cost.num);
		return game.checkMod(player, name, cost, select, 'gl_selectMaterial', player);
	}
	//获取动态消耗
	...
	get.gl_gouzhuList = function(player) {
		var list = [];
		for (var func of lib.gl_custom.cards) {
			func(player, list);
		}
	...
	//获取用于作为构筑素材的卡牌。
	get.gl_gouzhuCards = function(player, bool) {
		var list = [];
		if (bool) {
			var hs = player.getCards('hs');
			var es = player.getCards('e');
			if (hs.length) list.push(['你的手牌', hs]);
			if (es.length) list.push(['你的装备', es]);
			var cards = [];
			game.checkMod(player, cards, 'gl_gouzhuCards', player);
			for (var i = 0; i < cards.length; i++) {
				if (Array.isArray(cards[i])) {
					if (cards[i][1].length) list.push(cards[i]);
					cards.splice(i--, 1);
				}
			}
			if (cards.length) list.push(['其他', cards]);
		} else {
			for (var cards of game.checkMod(player, [], 'gl_gouzhuCards', player)) {
				if (Array.isArray(cards)) list.addArray(cards[1]);
			}
			list.addArray(player.getCards('hes'));
		}
		return list;
	}
	//开始构筑吧
	lib.skill.gl_gouzhu = {
		...
		chooseButton: {
			dialog: function(event, player) {
				var list = get.gl_gouzhuList(player);
			...
			select: function() {
				...
				return game.checkMod(player, [1, 1], 'gl_selectGouzhu', player);
			},
			filter: function(button, player) {
				var filter = lib.card[button.link[2]].gl_banGouzhu;
				if (filter && filter(player, _status.event)) return false;
				var mp = {};
				var buttons = ui.selected.buttons.slice(0);
				buttons.push(button);
				...
				var cards = get.gl_gouzhuCards(player);
				var selectedList = [];
				for (var cost of get.gl_material(button.link[2], player)) {
					if (get.countCards(cards, function(card) {
							if (get.gl_gouzhuEnabled(player, card, button.link[2], cost, selectedList)) {
								selectedList.push(card);
								return true;
							}
						}) < get.gl_selectMaterial(player, button.link[2], cost)[0]) return false;
				}
				return true;
			},
			...
			backup: function(links, player) {
				...
				if (evt.name == 'chooseToUse' && evt._aiexclude) evt._aiexclude.add('gl_gouzhu');
				...
				for (var name in cost) {
					for (var i of cost[name]) {
						if (get.gl_selectMaterial(player, name, i)[0] > 0) {
							directresult = null;
							break;
						}
						directresult.addArray(cards.filter(card => get.gl_gouzhuEnabled(player, card, name, i)));
					}
				}
				if (directresult !== null) {
					lib.skill.gl_gouzhu_backup_backup = {
						lose: false,
						links: links,
						cards: directresult,
						delay: false,
					...
					return lib.skill.gl_gouzhu_backup_backup;
				}
				return {
					chooseButton: {
						dialog: function(event, player) {
							var dialog = ui.create.dialog('请选择用于构筑的素材');
							for (var i of get.gl_gouzhuCards(player, true)) {
								dialog.add(i[0]);
								dialog.add([i[1], 'vcard']);
							}
							return dialog;
						},
						filterOk: function() {
							for (var name in cost) {
								for (var i of cost[name]) {
									var num = 0;
									var select = get.gl_selectMaterial(_status.event.player, name, i);
									var selectedList = [];
									for (var j of ui.selected.buttons) {
										if (get.gl_gouzhuEnabled(_status.event.player, j.link, name, i, selectedList)) {
											selectedList.push(j.link);
											num++;
										}
									}
									if (select[0] == -1) {
										if (get.countCards(cards, function(card) {
												return get.gl_gouzhuEnabled(player, card, name, i);
											}) != num) return false;
									} else {
						...
						select: function() {
							for (var name in cost) {
								for (var i of cost[name]) {
									if (get.gl_selectMaterial(_status.event.player, name, i)[0] > 0) {
										return [1, Infinity];
									}
								}
							}
						...
						filter: function(button, player) {
							for (var name in cost) {
								for (var i of cost[name]) {
									...
									var selectedList = [];
									for (var j of ui.selected.buttons) {
										if (get.gl_gouzhuEnabled(player, j.link, name, i, selectedList)) {
											selectedList.push(j.link);
											num++;
										}
									}
									var select = get.gl_selectMaterial(player, name, i)[1];
									if (select == -1) select = Infinity;
									if (select > num && get.gl_gouzhuEnabled(player, button.link, name, i, ui.selected.buttons)) return true;
								}
							}
							return false;
						},
		...
		contentx: function() {
			var names = [];
			var info = lib.skill.gl_gouzhu_backup_backup;
			for (var card of info.links) {
				names.push(card[2]);
			}
			var next = player.gl_gouzhu(names, info.cards);
